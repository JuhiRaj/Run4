...
bool EventCategorizer::init() {
  ...
  if (fSaveControlHistos) {
    ...
    //TOF Correction and Cut for the Tagged Scattered Hit

    getStatistics().createHistogram(new TH1F("TOF_Scat_Diff_real", "TOF_Scat_Diff_real",
      401, -200.5, 200.5));
    getStatistics().getHisto1D("TOF_Scat_Diff_real")->GetXaxis()->SetTitle("TOF_Scat_Diff_real[ns]");
    getStatistics().getHisto1D("TOF_Scat_Diff_real")->GetYaxis()->SetTitle("Counts");

    getStatistics().createHistogram(new TH1F("TOF_Scat_Diff_real_Cut", "TOF_Scat_Diff_real_Cut",
      401, -200.5, 200.5));
    getStatistics().getHisto1D("TOF_Scat_Diff_real_Cut")->GetXaxis()->SetTitle("TOF_Scat_Diff_real_Cut[ns]");
    getStatistics().getHisto1D("TOF_Scat_Diff_real_Cut")->GetYaxis()->SetTitle("Counts");
    ...
    //Cosine Spectra of the Angles

    getStatistics().createHistogram(new TH1F("Cosine12", "Cosine12",
      30, -1.5, 1.5));
    getStatistics().getHisto1D("Cosine12")->GetXaxis()->SetTitle("Cosine12");
    getStatistics().getHisto1D("Cosine12")->GetYaxis()->SetTitle("Counts");

    getStatistics().createHistogram(new TH1F("Cosine23", "Cosine23",
      30, -1.5, 1.5));
    getStatistics().getHisto1D("Cosine23")->GetXaxis()->SetTitle("Cosine23");
    getStatistics().getHisto1D("Cosine23")->GetYaxis()->SetTitle("Counts");

    getStatistics().createHistogram(new TH1F("Cosine31", "Cosine31",
      30, -1.5, 1.5));
    getStatistics().getHisto1D("Cosine31")->GetXaxis()->SetTitle("Cosine31");
    getStatistics().getHisto1D("Cosine31")->GetYaxis()->SetTitle("Counts");
...
double Calc3DAngle(const JPetHit & Hit1,
  ...
  double magProd = sqrt((pow(Hit1.getPosX(), 2) +
      pow(Hit1.getPosY(), 2) +
      pow(Hit1.getPosZ(), 2)) *
    (pow(Hit2.getPosX(), 2) +
      pow(Hit2.getPosY(), 2) +
...

{
  ...
  TVector3 k1(Hit1.getPosX(), Hit1.getPosY(), Hit1.getPosZ());
  TVector3 k1p(Hit2.getPosX() - Hit1.getPosX(), Hit2.getPosY() - Hit1.getPosY(), Hit2.getPosZ() - Hit1.getPosZ());
  TVector3 k2(Hit3.getPosX(), Hit3.getPosY(), Hit3.getPosZ());
...
//Calculate KN-Scattered Photon Energy

double CalcKN(double Energy_inc, double Angle3D)

{

  double pi = 3.141592653589793238512808959406186204433;
  double Mass_e = 510.99;
  double alpha = (Energy_inc) / Mass_e;
  double Energy_scat = (Energy_inc) / (1 + (alpha * (1 - cos(Angle3D * pi / 180.))));
  ...
  if (auto timeWindow = dynamic_cast <
    ...
    for (uint i = 0; i < n; ++i) {
      ...
      if (event.getHits().size() == 4) {
...

            {
...

              {
                ...
                if (fabs(Scat_Ordering_Vec.at(0).first) <= 0.5) {
                  ...
                  double pr1TOF = CalcTOF(pr1);
                  double pr2TOF = CalcTOF(pr2);
                  double pr3TOF = CalcTOF(pr3);
                  ...
                  double Scat_measured = (ScatHit.getTime() / 1000.0 - Scat_t1);
                  double TOF_Scat_Diff_real = Scat_measured - Scat_calculated;
                  getStatistics().getHisto1D("TOF_Scat_Diff_real")->Fill(TOF_Scat_Diff_real);

                  if (TOF_Scat_Diff_real <= 5.0 && TOF_Scat_Diff_real >= -5.0) {

                    getStatistics().getHisto1D("TOF_Scat_Diff_real_Cut")->Fill(TOF_Scat_Diff_real);

                    //3-Dimensional Angle Calculation between all four registered interactions
                    vector < pair < double, pair < JPetHit, JPetHit >>> Angle3D;
                    double Angle12 = Calc3DAngle(pr1, pr2);
                    double Angle23 = Calc3DAngle(pr2, pr3);
                    double Angle31 = Calc3DAngle(pr3, pr1);
                    Angle3D.push_back({
                      Angle12,
                      {
                        pr1,
		...
		if(Angle12 <= 180 && Angle23 <= 180 && AngleDiff <= 180)
		...
		{	
                    ...
                    double Energy1 = CalcEnergy1(Angle12, Angle23, (360 - (Angle12 + Angle23)));
                    double Energy2 = CalcEnergy2(Angle12, Angle23, (360 - (Angle12 + Angle23)));
                    double Energy3 = CalcEnergy3(Angle12, Angle23, (360 - (Angle12 + Angle23)));
                    ...
                    double Cosine12 = cos(Angle12);
                    double Cosine23 = cos(Angle23);
                    double Cosine31 = cos(Angle31);
                    getStatistics().getHisto1D("Cosine12")->Fill(Cosine12);
                    getStatistics().getHisto1D("Cosine23")->Fill(Cosine23);
                    getStatistics().getHisto1D("Cosine31")->Fill(Cosine31);
   ...
   getStatistics().getHisto1D("Sum_All_TOT_90")->Fill(Sum_All_TOT);
                      ...
                      double DistfromSurface = CalDistofSurface(pr1, pr2, pr3);
	...
	//Calculation of Expectation Value
                            ...
                            if (Scat_Ordering_Vec.at(0).second.first.getScintillator().getID() == Energy_Vector.at(2).second.getScintillator().getID()) {
                              ...
                              double ExpecValue1 = CalExpecValue(Scat_Ordering_Vec.at(0).second.first, Scat_Ordering_Vec.at(0).second.second, Energy_Vector.at(1).second);
                              ...
                              double ExpecValue2 = CalExpecValue(Scat_Ordering_Vec.at(0).second.first, Scat_Ordering_Vec.at(0).second.second, Energy_Vector.at(2).second);
                              ...
                              double ExpecValue3 = CalExpecValue(Scat_Ordering_Vec.at(0).second.first, Scat_Ordering_Vec.at(0).second.second,Energy_Vector.at(2).second);
